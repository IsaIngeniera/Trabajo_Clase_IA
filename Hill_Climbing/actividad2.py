...
def neighbors_of_state(height, width, houses, hospitals):
    ...
    moves = [(-1, 0), (1, 0), (0, -1), (0, 1)] #los mov que puede hacer 
...
def random_restart(height, width, houses, restarts=30, seed=0):
...
best, runs = random_restart(HEIGHT, WIDTH, houses, restarts=40, seed=SEED)
print('Mejor costo:', best['final_cost'])
print('Mejor k: ', best['k_hospital'])
...
def separarK (runs, num_hospital):
    ...
    for i in range(0, len(runs)):
        run = runs[i]
        if run['k_hospital'] == num_hospital:
            k.append(run)
    return k #devuelve una lista de diccionarios categoraizados por K


def generarTupla(nombreT1, nombreT2, k):
    costk=[]
    
    
    for i in k:
     tupla = []
     tupla.append( i[nombreT1] )   #usamos append porque la lista esta vacia
     tupla.append( i[nombreT2] )  
     #en cada vuelta, el diccionario actual se llama i

     costk.append(tupla)
      #para no guardar datos de bucles anteriores creamos la tupla en el mismo bucle para que no
      #tengamos que hacer clear, sino que el mismo bucle lo elimina
    return costk # devuielve una lista de tuplas
        


def minTupla(costk):
 """
 cuando recorremos listas grandes (una lista dentro de otra)
 la maquina primero escoge los elementos de primer nivel, 
 osea la lista interna que tenemos
 por lo tanto x seria la lista interna
 y solo falta poner la posicion del valor que queremos 
 """
 tupla1 = [] 
 tupla1 = min(costk, key=lambda x: x[1] ) 
 return tupla1 

stringHospital = 'k_hospital'
stringCosto = 'final_cost'

k1 = minTupla(generarTupla(stringHospital,stringCosto,(separarK(runs, 1))))
k2 = minTupla(generarTupla(stringHospital,stringCosto,(separarK(runs, 2))))
k3 = minTupla(generarTupla(stringHospital,stringCosto,(separarK(runs, 3))))
k4 = minTupla(generarTupla(stringHospital,stringCosto,(separarK(runs, 4))))
k5 = minTupla(generarTupla(stringHospital,stringCosto,(separarK(runs, 5))))     
...
plt.xlabel('k Hospitales')
plt.ylabel('Costo final')
plt.title('Variabilidad de hospitales')
plt.legend()
plt.grid(True)
plt.show()
